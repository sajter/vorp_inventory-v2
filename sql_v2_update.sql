...
/* to update to vorp inventory v2 */

ALTER TABLE `items` ADD COLUMN `durability` INT NULL DEFAULT NULL COMMENT 'Optional default/max durability';
ALTER TABLE `items_crafted` ADD COLUMN `durability` INT NULL DEFAULT NULL;
ALTER TABLE `items` ADD COLUMN `instructions` LONGTEXT NULL DEFAULT NULL COMMENT 'you can add a item instruction like where to find it how to use it' COLLATE 'armscii8_general_ci';
